   ...
   // function render(){
	...
	//(function() {
		...
		canvas.on('mouse:move', function(e) {
			...
			if(down) {
				if(e.transform) {
					offsetX = e.transform.offsetX || 0;
					offsetY = e.transform.offsetY || 0;
				}else {
				...
				objects.forEach(function(obj){
					if(obj.typegif == 'gif') {
						...
						var mask = $(obj.id),
							...
							group = obj.group, top, left, width, skewX, skewY,
						...
						/*if(group) {
							...
							skewX = obj.calcTransformMatrix()[3]
							left = obj.calcTransformMatrix()[4];
							top  = obj.calcTransformMatrix()[5];
							
							//console.log(group.angle);
							//console.log(obj.angle);
							
							//left = obj.calcTransformMatrix()[4] - width/2;
							//top  = obj.calcTransformMatrix()[5] - height/2;
							...
							transform = 'rotate('+ total +'deg) skewX('+ skewX +'deg) skewY('+ obj.skewY +'deg)';
							
							var maskStyle = '';
							
							if(group.flipX && !obj.flipX || !group.flipX && obj.flipX) {
								maskStyle += 'scaleX(-1) '
							}
							if(group.flipY && !obj.flipY || !group.flipY && obj.flipY) {
								maskStyle += 'scaleY(-1) '
							}
							if(maskStyle !='') {
