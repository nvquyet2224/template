		...
		function progressLog(byThis,kind) {
			if(!byThis._objects) {
				byThis.clone(function(object){
					...
					hisDatas.push(object);
					hisIndex = hisDatas.length-1;
					disable();
				});
			}
		}
		...
		function removeItem(id) {
			canvas.getObjects().forEach(function(o){
				if(o.id == id) { canvas.remove(o); return false; }
			});
		}
		function getItemPrev(id) {
			var result;
			for(var i = hisIndex-1;i>=0;i--) {
				if(hisDatas[i].id == id) {
					result = hisDatas[i];
					break;
				}
		...
		function undo() {
			...
			if(obj) {
				...
				if(obj.kind == 'add') {
					...
					var modify = getItemPrev(obj.id);
					if(modify) {
						modify.clone(function(o){
							var options = {id:modify.id, kind:modify.kind};
							modify.typegif && (options = {id:modify.id, kind:modify.kind, typegif:'gif'});
							o.set(options);
							canvas.add(o);
							o.typegif && updateMask(o);
						});
						
						//modify.typegif && updateMask(modify);
					}
					//canvas.setActiveObject(modify);
				}else if(obj.kind == 'delete') {
					
					obj.clone(function(o){
						var options = {id:obj.id, kind:obj.kind};
						obj.typegif && (options = {id:obj.id, kind:obj.kind, typegif:'gif'});
						o.set(options);
						canvas.add(o);	
						o.typegif && (addMask(o),updateMask(o));
		...
		canvas.on({
			'object:added': function(evt){
				if(!evt.target.id) {
					...
					evt.target.set({id:id, kind: 'add'});
					progressLog(evt.target,'add');
				}
			}, 'object:modified': function(evt) {
				evt.target.set({kind: 'modify'});
